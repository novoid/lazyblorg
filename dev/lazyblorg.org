...
# Time-stamp: <2014-03-02 19:30:37 vk>
...
** STARTED parse/htmlize: internal links                         :userstory:
...
- [ ] htmlizer.py: def sanitize_internal_links(self, sourcecategory, content):
  - error, if ID not found in blog_data!

- [ ] htmlizer_tests.py: def test_sanitize_internal_links(self):

- [ ] add section to testdata/currently_supported_orgmode_syntax.org
  - section including HTML result draft already exists but is tagged
    with NOEXPORT!

** STARTED implement: creating PERSISTENT pages                  :userstory:
:LOGBOOK:
...
- [X] time-ordered by last modification (for RSS and main page)
...
** NEXT impressum/about page                                     :userstory:
:PROPERTIES:
:CREATED:  [2014-02-09 Sun 10:33]
:BLOCKER: 2013-10-19-persistent-pages
:END:

- example: [[https://blog.fefe.de/impressum.html]]
- example: Tim Bray
  - http://www.tbray.org/ongoing/WhatItIs
  - http://www.tbray.org/ongoing/misc/Tim

** NEXT INFO: stats on files, headings and entries processed          :wish:
:PROPERTIES:
:CREATED:  [2014-02-09 Sun 10:31]
:END:

Example:
: 16 files (422933 lines) with 975 headings parsed
: resulted in 23 articles, 12 persistent pages, 79 tag pages
...
** TODO [#A] provide RSS feed(s)                                 :userstory:
:PROPERTIES:
