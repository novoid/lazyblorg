...
# Time-stamp: <2014-03-14 13:37:07 karl.voit>
...
** NEXT [#A] provide RSS feed(s)                                 :userstory:
:PROPERTIES:
...
- self-generating XML is not that hard to do
  - example: [[tsfile:2014-03-10-heise-example.rss]]
...
- [ ] time-ordered by issue day (for overview pages)
  ...
  - for each entry in entries
    ...
    - store to a sorted list (newest first or last)
** NEXT introduce a hidden tag                                   :userstory:
:PROPERTIES:
:CREATED:  [2014-03-12 Mi 17:54]
:END:

- articles tagged with the hidden tag don't show up in the RSS feed or
  the overview pages
