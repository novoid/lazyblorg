...
This is a web log (blog) environment for [[http://en.wikipedia.org/wiki/Emacs][GNU Emacs]] with [[http://orgmode.org/][Org-mode]] which
generates static HTML5 web pages. In my opinion, it is much more
superior to any other Org-mode-to-blog-solution I have seen so far
when it comes to minimal blogging effort and advanced features that
support my blogging experience.
...
See [[https://web.archive.org/web/20170909003450/https://article.gmane.org/gmane.emacs.orgmode/49747/][my original post to the Org-mode ML]] for how this idea of lazyblorg
started in 2011.
...
I definitely does not work with native Microsoft Windows.
...
Windows including the [[https://en.wikipedia.org/wiki/Windows_Subsystem_for_Linux][Windows Subsystem for Linux]] looks promising. It
might as well work as with any full-blown Linux system.
...
substantial changes to lazyblorg, you maybe will find [[https://karl-voit.at/tags/lazyblorg/][a blog article tagged
with "lazyblorg"]]. Use an RSS/Atom aggregator to follow the blog.

All updates and new features are visible in the [[https://github.com/novoid/lazyblorg/commits/master/][repository history]].
That's the most important source for updates.

As of 2025-12-13, I migrated this project to =uv= which resulted in
manifold changes related to the wrapper shell scripts, environment
variables used for the shell scripts and pytest invocation. This has
simplified stuff.
...
- lazyblorg offers so many cool little features as you can read in the
  "Features" section.
...
- lazyblorg is using state-of-the art HTML5 and CSS3
  ...
  - Results might not be compatible with very old browsers such as Internet
    Explorer or some mobile devices.
...
- Updating your lazyblorg setup after you've customized it requires
  manual steps so that you re-apply your customizations. See chapter
  "How to Upgrade lazyblorg to a Newer Version" below for the workflow.

- As lazyblorg is a works-for-me project: see chapter "DISCLAIMER:
...
- Multiple ways of adding comments: via good old email, Disqus and
  most important: *Mastodon*. (See [[https://karl-voit.at/2026/02/18/Vorschlag-minkorrekt-Fediverse-statt-Forum][this German article]] for details on
  how I think that Fediverse hashtags are a valid aproach to blog
  comments.)
...
- To efficiently notify users of new articles or changes to existing
  ...
  - See the tag pages and their TagTree drilldown for feeds on single
    tags or even combinations of tags!

- Really easy to use [[https://github.com/novoid/lazyblorg/wiki/Links#linking-other-blog-articles-internal-links][linking to other blog articles]] using their ID property.
...
- [[https://github.com/novoid/lazyblorg/wiki/Snippets][Snippets]]: Define reusable content fragments (small or very large) as
  Org-mode entries and reference them from any blog article using
  their ID property =id:snippet-id= as reference. Supports inline and
  block replacement, heading level adaptation, and nesting.
...
- You can very easily [[https://github.com/novoid/lazyblorg/wiki/Images][embed image files]] with automatically scaling to
  their desired width
  - This feature is hardened against moving image files to different
    folders!
...
- For navigating through the blog articles I do recommend using the
  ...
  - [[https://github.com/novoid/lazyblorg/wiki/TagTrees][TagTrees]] offer an advanced way of navigating through articles by
    combining multiple tags. Starting from any tag page, readers can
    drill down by selecting additional tags to narrow down the list of
    matching articles. This is ultra-cool if you grasp its power. For
    example, [[https://karl-voit.at/tags/emacs/education/][this page]] holds all articles tagged with "emacs" as well as
    "education". And you get an Atom feeds for that combination of tags
    as well.
...
- With the command line parameter =--external-url-file= you can
  generate a [[https://en.wikipedia.org/wiki/Tsv][TSV file]] listing all external URLs found across your blog
  articles. This is useful as input for external URL checker tools to
  detect broken links. The file includes the URL, the number of
  articles referencing it, the article IDs, and link descriptions.
...
I am using lazyblog for [[http://Karl-Voit.at][my own blog "publicvoit"]] and therefore it gets
more and more ready to use as I add new features for myself.
...
- [[https://github.com/astral-sh/uv][Python uv]] for dependency management and running all the important parts of lazyblorg: tests, blog generation process, ...
...
5. Get yourself an overview on *what defines a lazyblorg blog post* and
   ...
   4. A ~:LOGBOOK:~ entry has to be found with the time-stamp of
      ...
      - Note that you don't need to take care of the ~:CREATED:~ property as
        well as the ~:LOGBOOK:~ drawer as long as you're using following
        settings:
   ...
   5. Get yourself familiar on [[https://github.com/novoid/lazyblorg/wiki/Orgmode-Elements][the sub-set of Org-mode syntax you can use with lazyblorg]]
      ...
      - You should not get a disaster if you are using elements
        lazyblorg is not optimized for. The result just look different
        than expected.
      - However, "unknown" Org-mode elements are automatically converted
        through pandoc as a fall-back. So those cases should be extremely rare.
...
9. Publish your pages on a web space of your choice
   - publishing can be done in various ways. This is how I do it using
     ~lazyblorg/regenerate_public_voit.sh~ which is an
   ...
   - This method has the advantage that generating (invoking
     ~lazyblorg~) and publishing are separate steps. This way, I can
     locally re-generate the blog (for testing purposes) as often I
     want to. However, as long as I do not sync it to my web space, I
     keep the meta-data (which is in the local web space copy) of the
     published version (and not the meta-data of the previous
     test-run).
...
Since lazyblorg is a works-for-me project, there is no automated
upgrade mechanism. Upgrading requires a few manual steps, especially
if you have customized ~config.py~ or the templates.
...
If you have a custom CSS or SCSS file derived from the included one,
back that up as well:

: cp templates/public_voit.scss templates/public_voit.scss.MY_BACKUP

Also back up your invocation shell script if you based it on
...
If you get merge conflicts (because you edited tracked files like
~config.py~ directly), git will tell you. In that case, resolve the
conflicts manually or use ~git stash~ before pulling and ~git stash
pop~ afterwards.

** Step 3: Check for New or Changed Settings in config.py

This is the most important step. New versions of lazyblorg may
introduce new configuration variables in ~config.py~ that your old
copy does not have.
...
Look for:
- *New variables* that were added (you need to add them to your config
  with values matching your setup)
- *Changed default values* that might affect your blog
- *Removed or renamed variables* (rare, but possible)

Transfer your personal values (author name, domain, URLs, paths, ...)
from your backup into the new ~config.py~. Do *not* simply overwrite
the new ~config.py~ with your old backup -- you would lose any newly
introduced settings.

If I do need to merge/adapt very similar files, I prefer to use [[https://meldmerge.org/][Meld]]
for it. =meld file1 file2= opens up a GUI window where you see their
differences. Either via keyboard shortcuts or via mouse you're able to
move content from left to right or vice versa. Furthermore, you can
edit both files manually. Therefore, it gives you maximum flexibility
here.
...
New template elements or placeholder variables may have been added.
If you have customized the templates, merge the upstream changes into
your version carefully. Pay attention to new ~#PLACEHOLDER#~ variables
that the new code might expect.
...
This resolves any new or updated dependencies via ~uv~ and verifies
that lazyblorg starts without errors.
...
Generate your blog into a temporary directory to verify everything
looks correct before publishing:
...
Open a few generated pages in your browser and check that they look as
expected.

** Step 8: Clean Up

Once you are satisfied that everything works, you can remove the
backup files:
...
- Keep your customizations *minimal*. The less you change in
  ~config.py~ and the templates, the easier upgrades will be.
- Consider subscribing to [[https://karl-voit.at/tags/lazyblorg/][the lazyblorg blog tag]] via RSS/Atom to stay
  informed about important changes.
- If you track your lazyblorg setup in your own git repository, you
  can use ~git diff~ between versions to see exactly what changed
  upstream.
